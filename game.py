	...
	def __init__(self, maze: list):
		...
		for r in range(self.rows):
			for c in range(self.cols):
				...
				if (c, r) not in self.forty_two_coords\
					and (c, r) not in self.pacman.path\
					and (c, r) not in self.corners:
					self.all_cords.append((c, r))
		self.drawing_dots = sample(self.all_cords, min(len(self.all_cords), self.dots_number))
	...
	def on_draw(self):
		...
		for i in range(lives_remaining):
			arcade.draw_arc_filled(
				sidebar_x + 20 + (i * 45), self.height - 230, 32, 32, arcade.color.YELLOW, 30, 330
			)
		if self.pause:
			cx = self.width / 2
			cy = self.height / 2
