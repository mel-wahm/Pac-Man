...
class Game(arcade.View):
	def __init__(self, maze: list):
		...
		self.cell_size = min(
			max_maze_width / self.cols, (self.height - 100) / self.rows
		)
		# self.corners = {
		#     (0, 0),
		#     (self.cols - 1, 0),
		...
		self.drawing_dots = sample(
			self.all_cords, min(len(self.all_cords), self.dots_number)
		)
	...
	def on_key_press(self, symbol, modifiers):
		if self.intro and symbol != arcade.key.Q:
			self.intro = 0
			return
	...
	def on_update(self, delta_time):
		if not self.intro:
			...
			if not self.pause:
				...
				smooth_cell = (
					round(self.pacman.smooth_x),
					round(self.pacman.smooth_y),
				)
	...
	def on_draw(self):
		...
		if self.intro:
			...
			# Line 3: Pac-Man Life Icons on their own separate line below LIVES:
