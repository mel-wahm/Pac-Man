...
class Screen(arcade.View):
    def __init__(self):
        ...
        self.start_text = self.text_object(
            "START", (self.cx - 100, self.cy + 80)
        )
        self.settings_text = self.text_object(
            "SETTINGS", (self.cx - 100, self.cy)
        )
        self.exit_text = self.text_object(
            "EXIT", (self.cx - 100, self.cy - 80)
        )
        self.choice = 0
        self.choices = {0: "START", 1: "SETTINGS", 2: "EXIT"}
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == arcade.key.ENTER:
            if self.choice == 0:
                maze = MazeGenerator((15, 9)).maze
                game_view = Game(maze)
                self.window.show_view(game_view)
            if self.choice == 1:
                game_view2 = Settings(self)
