...
class MazeGenerator:
    def __init__(
        ...
        self._entryy = entry_cell[1] if 0 <= entry_cell[1] < self._height else 0
        self._exitx = (
            exit_cell[0] if 0 <= exit_cell[0] < self._width else self._width - 1
        )
        self._exity = (
            exit_cell[1] if 0 <= exit_cell[1] < self._height else self._height - 1
        )
    ...
    def _create_empty_maze(self) -> None:
        self._maze = [
            [8] + [0] * (self._width - 2) + [2] for _ in range(self._height - 2)
        ]
    ...
    def _is_available(self, x: int, y: int) -> bool:
        if 0 <= y < self._height and 0 <= x < self._width and self._path[y][x] == 0:
            return True
        return False

    def _get_neighbors(self, x: int, y: int) -> Iterator[tuple[int, int, int, int]]:
    ...
    def _find_short_path(self) -> None:
        directions = [(0, 1, 4, "S"), (1, 0, 2, "E"), (-1, 0, 8, "W"), (0, -1, 1, "N")]
        ...
        queue: deque[tuple[int, int, str]] = deque([(self._entryx, self._entryy, "")])
        while queue:
