...
class Ghost:
    ...
    def draw(self):
        ...
        arcade.draw_arc_filled(cx - 80 * s, cy - 75 * s, 80 * s, 80 * s, self.color, 180, 360)
        arcade.draw_arc_filled(cx + 80 * s, cy - 75 * s, 80 * s, 80 * s, self.color, 180, 360)

        # Eyes (Left & Right)
        for eye_x in (cx - 35 * s, cx + 35 * s):
            arcade.draw_circle_filled(eye_x, cy + 15 * s, 30 * s, arcade.color.WHITE, num_segments=32)
            arcade.draw_circle_filled(eye_x, cy + 15 * s, 20 * s, (33, 33, 255), num_segments=32)
            arcade.draw_circle_filled(eye_x, cy + 19 * s, 3 * s, arcade.color.WHEAT, num_segments=32)
